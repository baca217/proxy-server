...
void cache_file(char *url, char *cont);
...
int connect_http_server(char *url, char buf[MAXLINE], int clientfd)
{
	...
	read(sockfd, rec_buf, MAXLINE);
	if(strlen(rec_buf) == 0)
	{
		printf("connection to http server timed out for %s\n",url_cp);
		send_error(clientfd, 503);
		free(url_cp);
		free(port);
		close(sockfd);
		return 0;
	}
	write(clientfd, rec_buf, MAXLINE);
	cache_file(url_cp, rec_buf);
	free(url_cp);
	free(port);
	close(sockfd);
	return 0;
...
void cache_file(char *url, char cont[MAXLINE])
{
	unsigned long hash = 5381;
	int c;
	...
	if((fp = fopen(c_hash,"w")))
	{
		fwrite(cont, MAXLINE, 1, fp);
		fclose(fp);
	}
	else
	{
